...
func writeNonRootCmakeListsHeader(packageName core.PackageName, w io.Writer) error {
...
func writeNonRootCmakeListsBody(targetName core.TargetName, target core.Target, w io.Writer) error {
	switch target.(type) {
...
func makeNonRootCmakeLists(packageName core.PackageName, packageTargets *core.PackageTargets,
	...
	if err := writeNonRootCmakeListsHeader(packageName, f); err != nil {
		return err
	}

	for _, target := range packageTargets.TargetList {
		if err := writeNonRootCmakeListsBody(target.Label().Target, target, f); err != nil {
			return err
		}
	}
...
func makeAllCmakeLists(build *bazel.Build, outputPath string) error {
	...
	for packageName, packageTargets := range workspaceTargets {
		...
		if err := makeNonRootCmakeLists(packageName, packageTargets,
			packagePath); err != nil {
			return err
		}
	}
